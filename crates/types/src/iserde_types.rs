...
where
    Self: Sized,
{
    fn ideserialize<I: FeltIterator>(data: &mut I) -> Option<Self>;
    fn ideserialize_boxed<I: FeltIterator>(data: &mut I) -> Option<Box<Self>> {
        Self::ideserialize(data).map(Box::new)
    }
}

pub trait ISerdeItem {
    fn ideserialize_item<I: FeltIterator>(data: &mut I) -> Option<TypeDef>;
}
...
where
    Self: Sized,
{
    fn ideserialize_end<I: FeltIterator>(data: &mut I) -> Option<Vec<Self>>;
}
