...
use super::{Attribute, TypeDefStructured};
...
pub enum TypeDef {
    ...
    Tuple: Span<TypeDef>,
    Array: Box<TypeDef>,
    FixedArray: Box<FixedSizeArrayDef>,
...
pub impl MemberDefImpl of MemberDefTrait {
    fn new<T, impl TD: TypeDefStructured<T>>(
        name: ByteArray, attributes: Span<Attribute>,
    ) -> MemberDef {
        MemberDef { name, attributes, type_def: TD::type_def() }
    }
}
...
impl FixedArrayDefISerde of ISerde<FixedSizeArrayDef> {
    ...
    fn ideserialize(ref serialized: Span<felt252>) -> Option<FixedSizeArrayDef> {
        let type_def = ISerde::ideserialize(ref serialized)?;
