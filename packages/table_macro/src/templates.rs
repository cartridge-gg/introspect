...
pub fn table_impl_tpl(
    ...
    format!(
        "pub impl {table_impl} = {I_TABLE_PATH}::TableImpl<{meta_impl}, {primary_impl}, {columns_impl}>;"
    )
}

pub fn i_table_impl_tpl(table_impl: &str, i_table_impl: &str) -> String {
    format!("pub impl {i_table_impl} = {I_TABLE_PATH}::ITableImpl<{table_impl}>;")
}
...
pub fn member_impl_tpl(
    ...
    format!(
        "pub impl {member_impl} = {I_TABLE_PATH}::TableMemberImpl<{table_impl}, {columns_mod}::{member_name}, {type_str}>;\n"
    )
}
